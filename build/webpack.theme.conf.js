...
function getThemesEntry() {
  let entry = {};
  const dir = fs.readdirSync(path.resolve(__dirname, '../src/styles/themes'), {
    withFileTypes: true
  });
  for (const dirent of dir) {
    let name = dirent.name,
      entryName = name.split('.').slice(0, -1).join('.').toLowerCase();
    if (dirent.isFile() && !/^_/.test(name)) {
...
module.exports = {
  ...
  output: {
    path: path.resolve(__dirname, isSpaMode ? '../dist_spa' : '../dist'),
    filename: 'themes/js/[name].js',
    publicPath: isSpaMode ? '' : '/'
  },
  performance: {
    // 资源文件最大限制大小warning提示

...
export function base64Encode(str) {
  let aBytes = strToUint8(str),
    eqLen = (3 - (aBytes.length % 3)) % 3,
    sB64Enc = '';
...
function base64ToUint8(base64Content, nBlockSize) {
  let { noPaddingContent: sB64Enc, size: nOutLen } = getBase64Size(
      base64Content,
      nBlockSize
    ),
...
export function base64Decode(base64Content) {
  let sView = '',
    aBytes = base64ToUint8(base64Content);
...
function getBase64Size(base64Content, blockSize) {
  ...
  return {
    noPaddingContent,
    size: blockSize
      ? Math.ceil(((nopaddingLen * 3 + 1) >>> 2) / blockSize) * blockSize
      : (nopaddingLen * 3 + 1) >>> 2
  };
}
...
export function base64AsBlob(dataURI, mimeType) {
  let base64 = formatBase64(dataURI),
    buffer = base64ToUint8(base64.content);
